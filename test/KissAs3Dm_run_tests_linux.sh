...
KISSAS3FW_SRC=$(read_setting KISSAS3FW_SRC "the src folder of the KissAs3Fw checkout, for example /home/myUser/sources/KissAs3Fw/src")
BUILD="$ROOT/build/testrun/as3dm"

# The two values every application of this project is built with, see
# build/scripts/linux/setup.sh. The suites of one run work inside one single frame, so
...
run_test_application()
{
  ...
  set +e
  timeout 600 "$AIRSDK/bin/adl" "$descriptor"
  local adl_exit=$?
  set -e
